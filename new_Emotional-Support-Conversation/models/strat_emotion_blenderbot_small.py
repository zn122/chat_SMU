...
class Model(BaseModel, BlenderbotSmallForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs
        ...
        if validation:
            ...
            lm_logits = lm_logits[..., :self.toker.vocab_size+26].contiguous()
    ...
    def predict_strategy_emotion_and_intensity(self, logits, encoded_info):
        assert not self.training

        emotion_id = encoded_info.get('emotion_id', None)
        e_logits = logits[:, 0, -26:-14]
        ...
        intensity_id = encoded_info.get('intensity_id', None)
        i_logits = logits[:, 0, -14:-8]

        if intensity_id is not None:
            pred = intensity_id
        else:
            if SAMPLE:
                filtered_logits = top_k_top_p_filtering(i_logits / TEMPERATURE, top_p=0.9)
                pred = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1).squeeze(-1)
            else:
                pred = torch.argmax(i_logits, dim=-1)

        pred_top1 = torch.topk(i_logits, k=1, dim=-1)[1]
        pred_top3 = torch.topk(i_logits, k=3, dim=-1)[1]

        encoded_info.update({
            'pred_intensity_id': pred,
            'pred_intensity_id_top1': pred_top1,
            'pred_intensity_id_top3': pred_top3,
            'pred_intensity_id_dist': F.softmax(i_logits, dim=-1)
        })
    ...
    def generate(
        ...
        **kwargs
        ...
        self.predict_strategy_emotion_and_intensity(lm_logits, encoded_info)

        decoder_input_ids = torch.cat([decoder_input_ids,
                                       encoded_info['pred_emotion_id'][..., None] + len(self.toker) - 26,
                                       encoded_info['pred_intensity_id'][..., None] + len(self.toker) - 14,
                                       encoded_info['pred_strat_id'][..., None] + len(self.toker) - 8], dim=-1)  # -8 => 54962~54969
