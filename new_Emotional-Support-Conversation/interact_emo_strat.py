...
file_name = os.path.join('./history2.csv')
fieldnames = ['Date', 'time', 'speaker', 'text', 'emotion', 'intensity', 'strategy']
if not os.path.isfile(file_name):
    with open(file_name, 'a', newline='') as csvfile:
        wr = csv.writer(csvfile)
        wr.writerow(["Date", "Time", 'Speaker', "Text", "Emotion", "Intensity", "Strategy"])
#set_seed(args.seed)
...
while True:
    ...
    intensity_id_out = encoded_info['pred_intensity_id_top3'].tolist()[0][0]

    intensity = id2intensity[intensity_id_out]
    ...
    with open(file_name, 'a', newline='') as csvfile:
        wr = csv.writer(csvfile)
        wr.writerow([today, cur_t, 'sys',  text, emotion, intensity, strategy])
